...
a.loadTimeout=500;a.marketingCloudServer=a.audienceManagerServer="dpm.demdex.net"}Visitor.getInstance=function(k){var a,f=window.s_c_il,i;if(f)for(i=0;i<f.length;i++)if((a=f[i])&&"Visitor"==a._c&&a.namespace==k)return a;return new Visitor(k)};
    ...
    s.fpCookieDomainPeriods = window.location.hostname.split('.').length - 1;

    var omnitureHostName = window.location.host;

    var USProdRE = /www.esteelauder.com|m.esteelauder.com|espanol.esteelauder.com/i;
    if ( USProdRE.test( omnitureHostName )) {
        s.account = 'esteelaudernaus,esteeglobal'
        s.currencyCode ='USD'
    }

    var CAProdRE = /www.esteelauder.ca|m.esteelauder.ca|fresteelauderca.convertlanguage.com|francais.esteelauder.ca/i;
    if ( CAProdRE.test( omnitureHostName )) {
        s.account = 'esteelauderca,esteeglobal'
        s.currencyCode ='CAD'
    }


   /* if(($.cookie('LOCALE')) == 'fr_CA') { 
        tms_page_data.tms_page_info.site_info.LOCALE = "fr_CA";
        tms_page_data.tms_page_info.site_info.LANGUAGE_ID = "fr";
    }*/
    ...
    function s_doPlugins(s) {
        ...
        if (s.c_r('v24') != 'noclick' && s.c_r('v24') !=''){
            ...
            /* s.eVar24 = thisClick; */
            s.prop14="GNAV | " + tms_page_data.tms_page_info.page_name.pageID
            s.events = s.apl(s.events, 'event8', ',', 2)
            s.c_w('v24', 'noclick')
                           
	   }    
       
        if (s.c_r('v28') != 'nologin1' && s.c_r('v28')  !=''){
...
if (s.c_r('advancednightrepair') != 'noadvancednightrepair'){
   var coral = s.c_r('advancednightrepair')   
   s.eVar29= 'advancednightrepair|'
    if (!s.events){
        s.events='event16'
    }else{
        s.events = s.apl(s.events, 'event16', ',', 1)
    }
   s.c_w('advancednightrepair','noadvancednightrepair'); 
    
}
if (s.c_r('bronzegoddess') != 'nobronzegoddess'){
   var coral = s.c_r('bronzegoddess')   
   s.eVar29= 'bronzegoddess|'
    if (!s.events){
        s.events='event16'
    }else{
        s.events = s.apl(s.events, 'event16', ',', 1)
    }
   s.c_w('bronzegoddess','nobronzegoddess'); 
    
}if (s.c_r('cleardifference') != 'nocleardifference'){
   var coral = s.c_r('cleardifference')   
   s.eVar29= 'cleardifference|'
    if (!s.events){
        s.events='event16'
    }else{
        s.events = s.apl(s.events, 'event16', ',', 1)
    }
   s.c_w('cleardifference','nocleardifference'); 
    
}
if (s.c_r('purecolorenvy') != 'nopurecolorenvy'){
   var coral = s.c_r('purecolorenvy')   
   s.eVar29= 'purecolorenvy|'
    if (!s.events){
        s.events='event16'
    }else{
        s.events = s.apl(s.events, 'event16', ',', 1)
    }
   s.c_w('purecolorenvy','nopurecolorenvy'); 
    
}

s.setupLinkTracking = function($) {
    $(function() {
    	$('nav.page-navigation').on('click', 'a', function() {
